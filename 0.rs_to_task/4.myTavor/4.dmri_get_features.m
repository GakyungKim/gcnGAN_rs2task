...
%
% Extract the following features from diffusion MRI (projected onto the
% cortical surface)
%   1. Mean diffusivity (MD) calculated for 3 different b-values
...
% Assumes that diffusion tensor has been pre-calculated (e.g. with FSL's dtifit) on each shell
% and stored in a file called
% /diffdir/subjectID/T1w/Diffusion/dtifit_b{1,2,3}k/dti_tensor.nii.gz

% Project tensor onto cortical surface and open in matlab
...
for b=1:3
    % Create a pseudo-volume to perform tensor decomposition using FSL
    T = zeros(N,6);
